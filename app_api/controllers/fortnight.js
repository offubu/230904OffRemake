...
const yearfortnightsCreate = (req, res) => { let today = new Date(); let thisyear = today.getFullYear();
    ...
    for(let i=1;i<=27;i++){ fzero = new Date(yearStart.getTime() + (i-1)*1209600000);
        ...
        for(d=0;d<ndays;d++){ 
            ...
            if(todayDay==0 || todayDay==3){
                let iga = {  //  weekly/fornightly schedule, short list, including some that are like normal physcial activities, should always include a METs, eg for poker, you are sitting
                    ...
                    activityCode: 'g001',
                    duration: {
                        begin: new Date(today.getTime() + (18*60)*60000),  //  codesSet with time map, updates to time map or codeSet by various admins
                        end: new Date(today.getTime() + (21*60)*60000)
                    }
                }
                iGA.push(iga);
            }
